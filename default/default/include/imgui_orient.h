...
struct ImVec3 {
    ...
    float Dot(const ImVec3& b) const {
        return x * b.x + y * b.y + z * b.z;
    }
    ImVec3 Mult(float val) const {
        return ImVec3(x * val, y * val, z * val);
    }
    ImVec3 Div(float val) const {
        return ImVec3(x / val, y / val, z / val);
    }
    float Length() const {
...
struct ImQuat {
    ...
    ImQuat Div(float val) {
        return ImQuat(x / val, y / val, z / val, w / val);
    }
...
struct ImOrient {
    ...
    inline float QuatD(float w, float h) {
        return (float)std::min(abs(w), abs(h)) - 4.0f;
    }
